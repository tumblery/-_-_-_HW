...
int main(void){
        ...
        int push_dev,led_dev,fnd_dev,dot_dev,tlcd_dev,blt_dev;
	...
	if((blt_dev = open(BLT_DEV,O_RDWR))<0){
		printf("[-] ERROR: device open %s\n",BLT_DEV);
		return -1;
	}
	...
	printf("\t\t~~~WELCOM~~~\t\t\n");
	printf("\t1.LED(1's down, 2's upper, 3's all on, 4's all off\n");
	printf("\t2.Seven segment\n");
	printf("\t3.Dot matrix\n");
	printf("\t4.Text LCD\n");
        while(!quit){
		char SEL;
                read(blt_dev,&SEL,1);
                if(SEL =='1'){//if push button 1 about LED
			...
			led_btn = (++led_btn)%4;	
                ...
                else if(SEL=='2'){//if push button 2 about fnd
			...
			fnd_btn = (++fnd_btn)%2;
                ...
                else if(SEL == '3'){//if push button 3 about dot matrix
			...
			switch(dot_btn){
			 ...
			 dot_btn = (++dot_btn)%2;
                ...
                else if(SEL == '4') {//if push button 4 about text lcd
			...
			tlcd_btn = (++tlcd_btn)%2;
                ...
                else if(SEL == '5'){//if push button 5 about reset
